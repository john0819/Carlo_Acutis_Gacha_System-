# 快速启动指南

本文档说明如何在本地和服务器上快速启动和运行项目。

---

## 📍 本地开发环境

### 前置要求

- Go 1.19+
- Docker（用于运行PostgreSQL数据库）
- （可选）ngrok 或 cloudflared（用于公网访问）

### 快速启动步骤

#### 1. 启动数据库

```bash
./start_db.sh
```

这会启动PostgreSQL数据库容器。

#### 2. 启动开发服务器

**方式A：开发模式（推荐，显示实时日志）**
```bash
./dev.sh
```
- 端口：8080
- 前台运行，显示实时日志
- 按 `Ctrl+C` 停止

**方式B：后台运行**
```bash
# 启动数据库
./start_db.sh

# 后台运行服务器
go run main.go > server.log 2>&1 &
```

#### 3. 访问应用

- 登录页面：http://localhost:8080/login.html
- 主页：http://localhost:8080/index.html
- 健康检查：http://localhost:8080/health

#### 4. 停止服务

```bash
./stop.sh
```

---

## 🌐 本地开发 + 公网访问

如果需要让其他人访问你的本地服务，可以使用内网穿透工具。

### 🚀 一键启动（推荐，最简单）

**一键启动服务器 + 内网穿透 + 自动生成二维码：**

```bash
# 使用 ngrok（默认）
./start_with_tunnel.sh

# 或使用 Cloudflare Tunnel
./start_with_tunnel.sh cloudflare
```

这个脚本会自动完成：
- ✅ 启动数据库
- ✅ 启动服务器
- ✅ 启动内网穿透（ngrok 或 Cloudflare）
- ✅ 获取公网地址
- ✅ 自动打开二维码生成页面

**生成二维码：**
- 脚本会自动打开二维码生成页面
- 或者手动访问：`http://公网地址/qrcode.html`
- 二维码页面会自动检测当前公网地址并生成二维码

---

### 手动启动（分步操作）

#### 方式1：使用 ngrok

1. **安装 ngrok**
   ```bash
   # macOS
   brew install ngrok/ngrok/ngrok
   
   # 首次使用需要注册并配置token
   # 访问 https://dashboard.ngrok.com/signup
   ngrok config add-authtoken YOUR_TOKEN
   ```

2. **启动服务器**
   ```bash
   ./start_db.sh
   ./dev.sh
   # 在另一个终端窗口运行：
   ```

3. **启动 ngrok**
   ```bash
   ./start_ngrok.sh
   ```

4. **获取公网地址并生成二维码**
   - ngrok 会显示一个公网地址，例如：`https://xxxx.ngrok.io`
   - 访问：`https://xxxx.ngrok.io/qrcode.html` 生成二维码
   - 二维码页面会自动检测当前地址

#### 方式2：使用 Cloudflare Tunnel

1. **安装 cloudflared**
   ```bash
   brew install cloudflared
   ```

2. **启动服务器**
   ```bash
   ./start_db.sh
   ./dev.sh
   # 在另一个终端窗口运行：
   ```

3. **启动 Cloudflare Tunnel**
   ```bash
   ./start_cloudflare.sh
   ```

4. **获取公网地址并生成二维码**
   - Cloudflare 会显示一个公网地址
   - 访问：`公网地址/qrcode.html` 生成二维码

---

## 🖥️ 服务器部署

### 前置要求

- 服务器已安装 Docker
- 服务器已安装 Go（用于编译）
- （可选）已安装 Nginx（用于反向代理）

### 快速启动（测试模式）

适合快速测试，不需要Nginx和systemd服务。

#### 1. 配置SSH密钥（首次部署）

```bash
# 在本地运行
./scripts/setup_ssh_key.sh 服务器IP 用户名

# 如果自动配置失败，脚本会显示公钥内容
# 手动将公钥添加到服务器的 ~/.ssh/authorized_keys
```

#### 2. 上传代码到服务器

```bash
# 使用git克隆（推荐）
ssh admin@服务器IP
cd /home/admin
git clone https://github.com/your-repo/h5project.git
cd h5project

# 或者使用scp上传（如果代码有修改）
# 在本地运行：
scp -r . admin@服务器IP:/home/admin/h5project/
```

#### 3. 上传图片（如果需要）

```bash
# 在本地运行
./scripts/upload_images.sh 服务器IP 用户名
```

#### 4. 快速启动服务

```bash
# 在服务器上运行
cd /home/admin/h5project  # 或你的项目目录
./scripts/quick_start.sh
```

这会自动完成：
- ✅ 启动数据库
- ✅ 检查数据库连接
- ✅ 初始化数据库表结构
- ✅ 编译Go程序
- ✅ 启动服务（端口8080）

#### 5. 测试访问

```bash
# 在服务器上测试
curl http://localhost:8080/health

# 从外部访问（需要开放防火墙端口）
# http://服务器IP:8080
```

---

### 完整部署（生产环境）

适合正式部署，包括Nginx反向代理和systemd服务。

#### 1. 运行部署脚本

```bash
# 在服务器上运行
cd /home/admin/h5project
./deploy/deploy.sh
```

这会自动完成：
- ✅ 编译Go程序
- ✅ 创建项目目录（/opt/h5project）
- ✅ 配置systemd服务（开机自启）
- ✅ 配置Nginx反向代理
- ✅ 启动数据库
- ✅ 初始化数据库
- ✅ 启动服务

#### 2. 检查服务状态

```bash
# 检查Go服务
sudo systemctl status h5project

# 检查Nginx
sudo systemctl status nginx

# 查看日志
sudo journalctl -u h5project -f
```

#### 3. 开放防火墙端口

```bash
# 开放80端口（HTTP）
firewall-cmd --permanent --add-service=http
firewall-cmd --reload

# 或使用iptables
iptables -A INPUT -p tcp --dport 80 -j ACCEPT
```

#### 4. 访问应用

- http://服务器IP/
- http://你的域名/（如果配置了域名）

---

## 🔧 常用命令

### 本地开发

```bash
# 启动数据库
./start_db.sh

# 开发模式运行（显示日志）
./dev.sh

# 停止所有服务
./stop.sh

# 查看日志
tail -f server.log
```

### 服务器管理

```bash
# 快速启动（测试模式）
./scripts/quick_start.sh

# 启动数据库
./scripts/start_db_server.sh

# 初始化数据库和导入图片
./scripts/init_server.sh

# 完整部署
./deploy/deploy.sh

# 查看服务状态
sudo systemctl status h5project

# 查看日志
sudo journalctl -u h5project -f

# 重启服务
sudo systemctl restart h5project
```

---

## ❓ 常见问题

### 数据库连接失败

```bash
# 检查Docker容器是否运行
docker ps | grep h5project_db

# 查看数据库日志
docker logs h5project_db

# 重启数据库
docker-compose -f deploy/docker-compose.prod.yml restart
```

### 端口被占用

```bash
# 查看占用8080端口的进程
lsof -i :8080

# 停止进程
kill -9 PID
```

### 服务器上docker-compose命令找不到

新版本Docker使用 `docker compose`（无连字符）：
```bash
# 检查版本
docker compose version

# 使用新命令
docker compose -f deploy/docker-compose.prod.yml up -d
```

如果都没有，安装docker-compose：
```bash
yum install -y docker-compose
```

### 上传图片速度慢

脚本已自动使用 `rsync`（比scp更快）：
```bash
./scripts/upload_images.sh 服务器IP 用户名
```

如果还是慢，可能是网络问题，可以：
- 使用压缩传输（已自动启用）
- 分批上传
- 直接在服务器上下载图片

---

## 📝 项目结构说明

### 保留的脚本文件

**本地开发：**
- `start_db.sh` - 启动数据库
- `dev.sh` - 开发模式启动
- `stop.sh` - 停止服务
- `start_with_tunnel.sh` - **一键启动（数据库+服务器+内网穿透+二维码）** ⭐推荐
- `start_ngrok.sh` - ngrok内网穿透（单独使用）
- `start_cloudflare.sh` - Cloudflare内网穿透（单独使用）

**服务器部署：**
- `deploy/deploy.sh` - 完整部署脚本
- `scripts/quick_start.sh` - 快速启动（测试模式）
- `scripts/start_db_server.sh` - 启动数据库
- `scripts/init_server.sh` - 初始化服务器
- `scripts/upload_images.sh` - 上传图片到服务器
- `scripts/setup_ssh_key.sh` - 配置SSH密钥

---

## 🚀 快速参考

### 本地开发
```bash
./start_db.sh && ./dev.sh
```

### 本地 + 公网访问 + 二维码（一键启动）⭐推荐
```bash
./start_with_tunnel.sh
# 或指定内网穿透类型
./start_with_tunnel.sh ngrok      # 使用ngrok
./start_with_tunnel.sh cloudflare  # 使用Cloudflare
```

### 本地 + 公网访问（手动分步）
```bash
# 终端1
./start_db.sh && ./dev.sh

# 终端2
./start_ngrok.sh  # 或 ./start_cloudflare.sh

# 然后访问: 公网地址/qrcode.html 生成二维码
```

### 服务器快速测试
```bash
./scripts/quick_start.sh
```

### 服务器完整部署
```bash
./deploy/deploy.sh
```

